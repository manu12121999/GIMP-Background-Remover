...
class ResNet(nn.Module):
    ...
    def initialize(self):
        #self.load_state_dict(torch.load('./Ldf/resnet50-19c8e357.pth'), strict=False)
        self.load_state_dict(torch.load(os.path.join(os.path.dirname(__file__),'resnet50-19c8e357.pth')), strict=False)
...
class LDF(nn.Module):
    ...
    def initialize(self):
        if self.cfg.snapshot:
            self.load_state_dict(torch.load(self.cfg.snapshot,map_location='cpu'))
        else:
            weight_init(self)